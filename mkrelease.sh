...
cd $DIR/output/raspberrypi/target/var/www/web && sed -i 's/memcached/file/g' app/config/app.php && sed -i 's/\/data\/machinery\/config/\/\etc\/opt\/kerberosio\/config/g' app/config/app.php && tar czf $DIR/releases/rpi/$DATE/web.tar.gz .
...
cd $DIR/output/raspberrypi2/target/var/www/web && sed -i 's/memcached/file/g' app/config/app.php  && sed -i 's/\/data\/machinery\/config/\/\etc\/opt\/kerberosio\/config/g' app/config/app.php && tar czf $DIR/releases/rpi2/$DATE/web.tar.gz .
...
cd $DIR/output/raspberrypi3/target/var/www/web && sed -i 's/memcached/file/g' app/config/app.php && sed -i 's/\/data\/machinery\/config/\/\etc\/opt\/kerberosio\/config/g' app/config/app.php && tar czf $DIR/releases/rpi3/$DATE/web.tar.gz .