...
if [ "$target" == "mkimage" ]; then
    ...
    $boarddir/mkimage.sh
    cp $outputdir/images/kios-$board.img $basedir
    date=$(date +%Y%m%d)
    mv $basedir/kios-$board.img  $basedir/kios-$board-$date.img
    rm -f $basedir/kios-$board-$date.img.gz
    gzip $basedir/kios-$board-$date.img
elif [ -n "$target" ]; then
